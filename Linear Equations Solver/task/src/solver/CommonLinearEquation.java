...
public class CommonLinearEquation<T extends NumericElement<T>> {
    private NumericMatrixRow<T>[] rows;

    public CommonLinearEquation(NumericMatrixRow<T>[] rows) {
        this.rows = rows;
    }

    public void swapRows(int row1, int row2) {
        NumericMatrixRow<T> tmp = rows[row1];
        rows[row1] = rows[row2];
        rows[row2] = tmp;
    }
    ...
    public NumericElement<T> getElement(int row, int column) {
        return rows[row].get(column);
    }

    public List<NumericElement<T>> getColumn(int column) {
        List<NumericElement<T>> result = new ArrayList<NumericElement<T>>();
        for (int i = 0; i < rows.length; i++) {
            result.add(rows[i].get(column));
        }
        return result;
    }

    public List<NumericElement<T>> getLastColumn() {
        List<NumericElement<T>> result = new ArrayList<NumericElement<T>>();
        for (int i = 0; i < rows.length; i++) {
            result.add(rows[i].get(rows[i].size() - 1));
        }
        return result;
    }

    public void multiplyRow(int row, T value) {
...
//        rows[rowToWhichAdd].add(rows[rowToBeAdded].multiplied(coef));
    ...
    public NumericMatrixRow<T> getRow(int i) {
        return rows[i];
    }
